...
class Particle {
    constructor(x, y, color, speed, direction, life) {
        ...
        const angle = direction !== null ? direction + randomRange(-0.5, 0.5) : randomRange(0, Math.PI * 2);
...
class Player {
    ...
    update(dt, height) {
        ...
        const targetAngle = (this.velocity / 600) * 45 * (Math.PI / 180) * this.gravityDirection;
    ...
    draw(ctx) {
        ...
        if (Math.random() > 0.6) {
            ...
            ctx.beginPath();
            ctx.arc(-16 - Math.random() * 5, 0, 2 + Math.random() * 2, 0, Math.PI * 2);
            ctx.fill();
        }
...
class Game {
    ...
    gameOver() {
        ...
        const highScore = parseFloat(localStorage.getItem('stellarDriftHighScore')) || 0;
    ...
    update(dt) {
        ...
        if (this.player.y <= this.player.radius || this.player.y >= this.height - this.player.radius) {
            if (this.player.shieldTime > 0) {
                ...
                this.player.y = this.player.y <= this.player.radius ? this.player.radius + 1 : this.height - this.player.radius - 1;
                this.player.shieldTime = 0;
                this.particles.spawn(this.player.x, this.player.y, CONFIG.COLORS.shield, 20, 100);
                this.audio.playCrash();
            } else {
